...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root@localhost:3306/test3'
...
class EntitySchema(ma.Schema):
    class Meta:
        # Fields to expose
        fields = ('id','name','subsidiary_name','domain_name','zone','type','location','datastore_id','unq_row_id','create_ts','update_ts')
...
class RuleAssignmentSchema(ma.Schema):
    class Meta:
        # Fields to expose
        fields = ('id','description','rule_type_id','send_alert_flg','stop_job_flg','target_entity_id','source_entity_id','create_ts','update_ts','store_result_to_db_flg')
...
def add_ruleassig():
    ...
    new_ruleassig_params = RuleAssignment(description,rule_type_id,send_alert_flg,stop_job_flg,target_entity_id,source_entity_id,store_result_to_db_flg)
...
class RuleLogSchema(ma.Schema):
    class Meta:
        # Fields to expose
        fields = ('id','rule_assignment_id','rule_set_assignment_id','data_dt','rule_start_ts','rule_end_ts','batch_dt', 'target_sql_query', 'source_sql_query','target_result_value','source_result_value','result','status','partition_type','seq_num','create_ts','update_ts')
...
def add_rulelog():
    ...
    new_rulelog = RuleLog(id,rule_assignment_id,rule_set_assignment_id,rule_end_ts,batch_dt,target_sql_query,source_sql_query,target_result_value,source_result_value,result,status,partition_type,seq_num)
...
@app.route('/status/<user>/<rulesetname>/<date_time>')
def process(user,rulesetname,date_time):
    rulesetname_datetime = rulesetname + "_" + date_time
    result = db.session.query(RuleSet.name).filter_by(name=rulesetname).first()
    if result is not None:
       results = str(result[0]) + "_" + date_time

       if bool(mylist) is False :
          rulesetname_datetime=rulesetname+"_"+date_time
          print(rulesetname_datetime)
          mylist.setdefault(user,[]).append(rulesetname_datetime)
          print('first element added')
          a = command+rulesetname_datetime.replace("_"," ")+" /home/rvi/revas.log &"
          result_status = subprocess.Popen(a, shell=True)
          print(a)

          print(mylist)

          return "you are:==>"+str(user)+"\tand you are running script with params:==>"+str(rulesetname_datetime)+'\t time:'+str(datetime.datetime.now())


       elif  mylist.get(user) is None:
             rulesetname_datetime = rulesetname + "_" + date_time
             mylist.setdefault(user, []).append(rulesetname_datetime)
             print("new key")
             print(mylist)
       ...
       elif  rulesetname_datetime in mylist.get(user):
              rulesetname_datetime = rulesetname + "_" + date_time
              # check if it is still running with grep command
              #print still running plz wait
              #if not running in shell goto database and check its done or not if done remove from dict
              save=process_get(rulesetname)
              if "running" in save:
               print("ist running")
               print(mylist)
               print("find ya adn poping")
               return "hey==>"+user+"\tYour jobs==>"+rulesetname_datetime+"\tis running in background please wait before starting it again"
              else:
               print("not running ")
               #mylist.get(user).remove(rulesetname_datetime)
               rulesetname_datetime = rulesetname + "_" + date_time
               #mylist.setdefault(user, []).append(rulesetname_datetime)
               a = (command + rulesetname_datetime).replace("_", " ") + " /home/rvi/revas.log &"
               result_status = subprocess.Popen(a, shell=True)
               print(mylist)
               print("started job")
               return "As this jobs has already executed sucessfully stating it again"+"username:==>"+user+"and rulesetname:==>"+rulesetname_datetime

       else:
          print("added new value in old key")
          rulesetname_datetime = rulesetname + "_" + date_time
          mylist.setdefault(user, []).append(rulesetname_datetime)
          print(mylist)
          a = command+rulesetname_datetime.replace("_"," ")+" /home/rvi/revas.log &"
...
'''
    ...
    result = db.session.query(RuleSet.name).filter_by(name=rulesetname).first()
    print("reult is")
    a = command + " " +rulesetname+"/home/rvi/revas.log &"
    print(a)
    if result is not None and result[0] not in mylist:
     mylist.append(rulesetname)
     #result_status = subprocess.Popen(a,shell=True,std
     result_status=subprocess.Popen(a,shell=True)
     save=process_get(rulesetname)
     print(save)
     #output,error=result_status.communicate()
     #os.waitpid(.pid,0)
...
def process_get(rulesetname):
    ...
    if "bash" in str(result):
      return "its running "
        ...
        result = db.session.query(RuleLog.status).filter_by(id=rulesetname).first()
        result="done"
        return "job was done sucessfully"
